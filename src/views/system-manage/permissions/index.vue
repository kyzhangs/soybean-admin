...
async function getRolePermissions() {
  ...
  startLoading();
  const params = { role_code: RoleCode.value };
  const { error, data } = await fetchGetRolePermissions(params);
  if (!error) {
...
async function doSavePermissions() {
  const data: Api.SystemManage.PermissionsUpdateParams = {
  ...
  const { error, data: result } = await fetchUpdateRolePermissions(params, data);
  if (!error) {
